...
DEFAULT_DTYPE_CREATE_THUMBNAIL = \
    '''convert -resize 160x $file_name $thumbnail_file_name'''
...
class Dataset(models.Model):
    ...
    class Meta:
        permissions = (('dataset_colaborate',
...
class DataType(models.Model):
    ...
    create_thumbnail_command = \
        models.TextField(default=DEFAULT_DTYPE_CREATE_THUMBNAIL)
    acceptable_mimtypes_regex = models.CharField(max_length=256,
            default=r'.*')
...
class Datum(models.Model):
    ...
    created = CreationDateTimeField(_('created'))
    name = models.CharField(max_length=256)
    package = models.FileField(_('file'),
                               upload_to=get_package_file_path('datum'
                               ), max_length=100,
                               storage=protected_storage)
    dtype = models.ForeignKey(DataType, verbose_name=_('Type'),
                              related_name='+', blank=False, default=1)
    ...
    @models.permalink
    def get_absolute_url(self):
        return ('datasets_datum_detail', (), {'dataset_id': self.dataset_id,
            'pk': self.pk})
    ...
    def file_url(self):
        return reverse('datasets_datum_file',
                kwargs={'dataset_id': self.dataset_id, 'pk': self.pk})

    def thumbnail_url(self):
        url = reverse('datasets_datum_thumbnail',
                kwargs={'dataset_id': self.dataset_id, 'pk': self.pk})
        return url

    def file_name(self):

        return slugify(self.name) + '.' + self.package.name.split('.'
                )[-1]

    def file_render(self):
        template = string.Template(self.dtype.template_to_view)
        text = template.safe_substitute(file_url=self.file_url(),
                file_name=self.file_name())
        return text

    def get_thumbnail_file_path(self):
        return self.dtype.create_thumbnail(self.package.path,
                'datum_thumbnails')
...
class DatumAttachment(models.Model):
    ...
    name = models.CharField(max_length=256)
    package = models.FileField(_('file'),
                               upload_to=get_package_file_path('attachment'),
                               max_length=100,
                               storage=protected_storage)
    ...
    @models.permalink
    def get_absolute_url(self):
        return ('datasets_datumattachment_detail', (),
                {'dataset_id': self.datum.dataset_id,
                    'datum_id': self.datum_id, 'pk': self.pk})
    ...
    def file_url(self):
        return reverse('datasets_datumattachment_file',
                       kwargs={'dataset_id': self.datum.dataset_id,
                       'datum_id': self.datum_id, 'pk': self.pk})
