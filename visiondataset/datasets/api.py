...
from djangorestframework.views import ListOrCreateModelView,\
        InstanceModelView, ListModelView, View
...
class DatumResource(ModelResource):
    ...
    def file(self, instance):
        return reverse('api_datum_file', kwargs={'id': instance.id})

    def attachments(self, instance):
        return reverse('api_attachment_list', kwargs={'datum': instance.id})
...
class DatumAttachmentResource(ModelResource):
    ...
    def file(self, instance):
        return reverse('api_attachment_file', kwargs={'id': instance.id})
...
class DatumFileApiView(View):
    ...
    def get(self, request, id):
        ...
        return sendfile(request, self._datum.package.path,
                        attachment_filename=self._datum.file_name(),
                        attachment=True)
...
class DatasetsApiView(ListModelView):
    ...
    def get_queryset(self):
        queryset = get_objects_for_user(self.user, 'dataset_colaborate',
                                        klass=Dataset)
        return queryset
...
class DatumAttachmentsApiView(ListOrCreateModelView):
    ...
    def get_dataset(self):
        datum = get_object_or_404(Datum, pk=self.kwargs['datum'])
        dataset = datum.dataset
        return dataset

    def get_instance_data(self, model, content, **kwargs):
        data = super(DatumAttachmentsApiView, self).get_instance_data(model,
                content, **kwargs)
